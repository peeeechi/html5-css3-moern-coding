...
メイン領域は、そのコンテンツのまとまりから_Hot topic_、 _News_、 _Articles_の3ブロックから成る構成にする。
また、ブロック毎のタイトルは`h2`要素で表現するが、実装しようとしているデザインでは _News_ 以外のブロックのブロックタイトルがデザイン上は存在しない。
この場合、_News_のみに`h2`要素を書くと、_News_セクションの中に_Articles_セクションが取り込まれてしまい、アウトラインが変わってしまう。
これを防止するため、_Hot topic_、 _News_、 _Articles_セクションタイトルを`h2`要素として記述し、CSSで非表示にする対応をとる。
